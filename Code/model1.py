...
if __name__ == '__main__':
  ...
  def decay_triangle(state, dt, w, W):
    ...
    return state
  def w3(state, model):
    return gamma*state["Triangle"]/model.V
  ...
  def decay_circle(state, dt, w, W):
    ...
    return state
  def w4(state, model):
    return gamma*state["Circle"]/model.V

  def enzyme_circle(state, dt, w, W):
    p = np.random.poisson(W*dt)
  ...
  def enzyme_triangle(state, dt, w, W):
    p = np.random.poisson(W*dt)
    state["Enzyme"] = state["Enzyme"] - 1
  ...
  def enzyme_circle_unbinding(state, dt, w, W):
    ...
    return state
  def w7(state, model):
    return nabla_minus*state["Enzyme + Circle"]/model.V
  ...
  def enzyme_triangle_unbinding(state, dt, w, W):
    ...
    return state
  def w8(state, model):
    return nabla_minus*state["Enzyme + Triangle"]/model.V
  ...
  solver = Solver([X1,X2,E,C1,C2], initial=np.array([0,20,20,5,0,0]))
